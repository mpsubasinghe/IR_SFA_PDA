...
public class SPMktRetActivity extends AppCompatActivity {
    String[][] DiscountSpecial = new String[50][8];
      ...
      //  textpword= (EditText) findViewById(R.id.editTextcode);
        ...
        spinnerType.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            ...
            public void onItemSelected(AdapterView<?> arg0, View view, int arg2, long arg3) {
                ...
                //   String selected_val=spinner.getSelectedItem().toString();
                Rt_type_Des=  spinnerType.getItemAtPosition(spinnerType.getSelectedItemPosition()).toString();
                if (Rt_type_Des.equals("Sound"))
                {
                    Rt_type="S";
                }else if (Rt_type_Des.equals("Damage"))
                {
                    Rt_type="D";
                }else if (Rt_type_Des.equals("Expire"))
                {
                    Rt_type="E";
                }
                else if (Rt_type_Des.equals("St.Exp"))
                {
                    Rt_type="T";
                }
...
//                }
                ...
                DiscountSpecial[Tcount][7] = Rt_type;
...
//                }
                ...
                msgtext ="Returned Approval For Special \n";
                msgtext = msgtext +" Discounted Retailer \n";
                msgtext = msgtext + stname + "\n";
                msgtext = msgtext + repname + "\n";
                msgtext = msgtext + MyGloble.RName + "\n";
//                msgtext = msgtext + "Total RD ( " + CategeoryName + " )- " + RDtot + "\n";
//                msgtext = msgtext + "Total Discount ( " + CategeoryName + " )- " + RDDis + "\n";
//
//                msgtext = msgtext + "Requesting Discount - Rs. " + Dis + " @ " + textDis.getText().toString() + "%" + "\n";
//                msgtext = msgtext + "Requesting Invoice Value - Rs. " + (tot - Dis) + " @ " + textDis.getText().toString() + "%" + "\n";
//                msgtext = msgtext + "Requesting Products \n";

                for (int i = 0; i < DiscountSpecial.length; i++) {
                    if (!isEmptyString(DiscountSpecial[i][0])) {
                        msgtext = msgtext + DiscountSpecial[i][6] + " - " + DiscountSpecial[i][1] + " - " + DiscountSpecial[i][2]+ " - " + DiscountSpecial[i][7] + " \n";
                    }
                }
                msgtext = msgtext + "Code : ";

                pword = getRandomString(6);
                msgtext = msgtext+pword;


                sendMessage(B_Mgr_Tp, msgtext);
...
//                }
                ...
                LayoutInflater li = LayoutInflater.from(SPMktRetActivity.this);
                View promptsView = li.inflate(R.layout.custom, null);

                AlertDialog.Builder alertDialogBuilder = new AlertDialog.Builder(
                        SPMktRetActivity.this);
                ...
                final Button   btn1 = (Button) promptsView
        ...
        Btnback.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                Intent intent = new Intent(SPMktRetActivity .this, InvmainActivity .class);
                SPMktRetActivity.this.startActivity(intent);
...
//
    ...
    private void ConformPassword()
    {
        Cursor cursor1=db.GetC("select ReturnMethod,PDAMarketReturn.Qty,PDAProduct.Price,PDAMarketReturn.ProductID,PDAMarketReturn.FID,PDAMarketReturn.FIQty from PDAMarketReturn,PDAProduct WHERE PDAProduct.ProductID=PDAMarketReturn.ProductID AND  PDAProduct.ProCatNo='"+CategCode+"' AND   InvID='"+ MyGloble.InvoiceNo+"'   ");
        if( cursor1.getCount() > 0) {
            if (cursor1.moveToFirst())
            {
                while (!cursor1.isAfterLast()) {
                    if (cursor1.getString(cursor1.getColumnIndex("ReturnMethod")).equals("S")) {
                        MyGloble.db.execSQL("Update PDAProduct SET Quantity=Quantity - '" + cursor1.getString(cursor1.getColumnIndex("Qty")) + "' WHERE ProductID = '"+cursor1.getString(cursor1.getColumnIndex("ProductID"))+"'" );
                        MyGloble.db.execSQL("Update PDAProduct SET Quantity=Quantity - '" + cursor1.getString(cursor1.getColumnIndex("FIQty")) + "' WHERE ProductID = '"+cursor1.getString(cursor1.getColumnIndex("FID"))+"' " );
                    }
                    MyGloble.db.execSQL("DELETE From PDAMarketReturn WHERE ProductID = '"+cursor1.getString(cursor1.getColumnIndex("ProductID"))+"' AND  InvID='"+ MyGloble.InvoiceNo+"'  " );
                    cursor1.moveToNext();
                }
            }
        }
        ...
        for (int i = 0; i < DiscountSpecial.length; i++) {
            if (!isEmptyString(DiscountSpecial[i][0])) {
                ...
                MyGloble.db.execSQL("insert into PDAMarketReturn values ('"+MyGloble.InvoiceNo+"' ,'"+ DiscountSpecial[i][0]  +"','"+DiscountSpecial[i][2]+"','"+DiscountSpecial[i][3]+"','"+DiscountSpecial[i][7]+"','"+DiscountSpecial[i][1]+"','-1','0','"+String.format("%.2f",Double.valueOf(DiscountSpecial[i][4]))+"','"+String.format("%.2f",Double.valueOf(DiscountSpecial[i][5]))+"','"+RetInvNo+"','Y')" );
                if (DiscountSpecial[i][7].equals("S")) {
        ...
        if( cursor7.getCount() > 0) {
            MyGloble.db.execSQL("Update PDAInvoice SET MktReturn='" + MyGloble.TotalMKTR +"' WHERE InvoiceID='"+MyGloble.InvoiceNo+"' " );
        }else {
    ...
    public void DialogBoxOneButton(String msg, String head) {
        AlertDialog alertDialog = new AlertDialog.Builder(SPMktRetActivity.this).create();
    ...
    public void sendSMS1(String phoneNo, String msg) {
        try {
